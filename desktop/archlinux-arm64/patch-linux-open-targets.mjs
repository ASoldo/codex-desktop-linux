...
function findOpenTargetsRegistry(source) {
  const registryPatterns = [
    {
      kind: "array",
      pattern:
        /var ([A-Za-z_$][\w$]*)=\[((?:[A-Za-z_$][\w$]*,?)+)\]((?:,[A-Za-z_$][\w$]*=[A-Za-z_$][\w$]*\.[A-Za-z_$][\w$]*\(`open-in-targets`\)|;[A-Za-z_$][\w$]*\.[A-Za-z_$][\w$]*\(`open-in-targets`\));)/,
    },
    {
      kind: "map",
      pattern:
  ...
  for (const { kind, pattern } of registryPatterns) {
...
const jsFiles = readJsFiles(buildRoot);
const targetFiles = jsFiles.filter((file) => {
  const candidateSource = readFileSync(file, "utf8");
  return (
    candidateSource.includes("open-in-targets") &&
    requiredMarkers.every((marker) => candidateSource.includes(marker)) &&
    findOpenTargetsRegistry(candidateSource) != null
  );
});

if (targetFiles.length < 1) {
  fail(`expected at least one open-in-targets bundle, found ${targetFiles.length}`);
}
...
function findRequireBinding(source, moduleName) {
  ...
  const pattern = new RegExp(
    String.raw`(?:^|[;,])(?:let |const |var )?([A-Za-z_$][\w$]*)=require\((?:["']node:${escapedModuleName}["']|\x60node:${escapedModuleName}\x60)\)`,
  );
  return source.match(pattern)?.[1] ?? null;
}

function makeLinuxPatch({ codeArgsName, fsName, osName, pathName, openPathMatch }) {
  const openFileManagerPath = openPathMatch
    ? `function linuxOpenFileManagerPath(e){let t=e;for(;;){if((0,${fsName}.existsSync)(t))break;let e=(0,${pathName}.dirname)(t);if(e===t){t=null;break}t=e}let n=t??e;if((0,${fsName}.existsSync)(n)&&(0,${fsName}.statSync)(n).isFile())n=(0,${pathName}.dirname)(n);return ${openPathMatch[1]}(n)}`
    : `async function linuxOpenFileManagerPath(e){let t=e;for(;;){if((0,${fsName}.existsSync)(t))break;let e=(0,${pathName}.dirname)(t);if(e===t){t=null;break}t=e}let n=t??e;if((0,${fsName}.existsSync)(n)&&(0,${fsName}.statSync)(n).isFile())n=(0,${pathName}.dirname)(n);let{shell:r}=await import(\`electron\`),i=await r.openPath(n);if(i)throw Error(i)}`;
...
for (const targetFile of targetFiles) {
  ...
  const registryMatch = findOpenTargetsRegistry(source);
  if (!registryMatch) {
  ...
  for (const targetId of expectedTargets) {
    if (!source.includes(`id:\`${targetId}\``)) {
      fail(`upstream bundle layout changed in ${targetFile}; missing target id: ${targetId}`);
    }
  }
  ...
  const linuxPatch = makeLinuxPatch({
    codeArgsName: codeArgsMatch[1],
    fsName,
    osName,
    pathName,
    openPathMatch: source.match(openPathPattern),
  });

  const { kind, source: registrySource, registryName, registryEntries, suffix: registrySuffix } =
    registryMatch;
  const replacement =
    kind === "map"
      ? `${linuxPatch}var ${registryName}=new Map([${linuxTargets},${registryEntries}].flatMap(e=>{let t=e.platforms[process.platform];return t==null?[]:[[e.id,{id:e.id,...t}]]}));`
      : `${linuxPatch}var ${registryName}=[${linuxTargets},${registryEntries}]${registrySuffix}`;
...
if (patchedCount === 0) {
  console.log("patch-linux-open-targets: all open-targets bundles already patched");
}
